...
command -v duti >/dev/null 2>&1 ||
    {
        echo >&2 "duti required: brew install duti"
        exit 1
    }
...
assoc "$TEXT_EDITOR" "editor" txt md js jse json jsonc reg bat ps1 cfg sh bash yaml py ts tsx vue go java xml xsd log makefile sql csv
